...
For $v \in TM = \bigsqcup_{p \in M} T_p M$, if $1 \in J_v$, define $\exp v := \gamma_v(1)$.
...
$\exp_p v \quad (v \in D_p)$ is the point reached by traveling along the geodesic from $p$ in direction $v$ for the distance $\|v\|$.
$\forall v \in D_p$ and $\forall t \in [0,1]$, $1 \in J_v = t J_{tv} \subset J_{tv} \implies tv \in D_p$.
...
\begin{problem}
    Provide details for the proof of \autoref{lemma:10.4}.
...
$\forall p_0 \in M$, there exists an open neighbourhood $O \subset M$ of $p_0$ and $r >0$ s.t. the following hold
\begin{enumerate}[label=(\arabic*)]
\item $\forall p \in O$, $B(0_p,r) \subset D_p$ and $\exp_p:B(0_p,r)\to B(p,r)$ is a diffeomorphism.
\item $\forall p \in O$ and $\forall q \in B(p,r)$, there exists a unique piecewise smooth minimizing curve
