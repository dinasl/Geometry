...
\begin{definition}
    ...
    The map $exp : D \to M$ is the exponential map.
    For $p \in M$, let $D_p := D \cap T_pM$ and $exp_p := exp|_{D_p}$.
    The map $exp_p : D_p \to M$ is the exponential map at $p$.
...
\begin{lemma}
    ...
    \[
    \begin{aligned}
        f(v) := \left( \pi(v), exp v\right), v \in D
    \end{aligned}
    \]
    $\forall p_0 \in M$, $\exists$ open neighbourhood $0_{p_0} \subset TM$ of $0_{p_0}$ such that
    \begin{enumerate}
        \item $0_{p_0} \subset D$
        \item $f(0_{p_0})$ is open in $M \times M$
        \item $f : 0_{p_0} \to f(0_{p_0})$ is a diffeo.
    \end{enumerate}
    \label{lemma:10.4}
\end{lemma}
...
\begin{proof}
    ...
    Local expression $\bar{f} := (\phi \times \phi) \circ f \circ \tilde{\phi}^{-1}$ is:
    \[
...
Thus $exp_p: T_p M \cap 0_{p_0} \to Q_p$ is a diffeo. Since $B(0_p,r) \subset 0_{p_0}$, (2) and (3) follow.
\end{proof}
...
\begin{proof}
    (1) follows from $\gamma_v'(1) = \frac{d}{dt} exp_p (tv) |_{t=1}= d(exp_p)_v v$.
    
    (2) Let $ p \in M$ and $v,w \in T_pM$. Case $v=0_p$ is trivial.
