...
\begin{proof}
    ...
    \[
    \begin{aligned}
        \frac{d^2}{ds^2} \|T\| |_{s=0} =& \frac{1}{\|c_0'(0)\|} \Bigg(\frac{d}{dt} \langle \nabla^c_{\frac{\partial}{\partial t}} X, T \rangle - \langle R(X,T)T,X \rangle \\
        &+ \langle \nabla_{c_0} X, \nabla_{c_0}X \rangle - \langle \nabla_{c_0}X, \frac{T}{\|T\|} \rangle^2\Bigg).
    \end{aligned}
    \]
...
\[
\begin{aligned}
    \operatorname{diam}(M) := \sup_{p,q \in M} d(p,q) \qquad (\leq + \infty)
\end{aligned}
\]
...
\begin{theorem}[Bonnet-Myers theorem]
    ...
    Then $M$ is compact and $\operatorname{diam}(M) \leq \frac{\pi}{\sqrt{k}}$
    \label{thm:13.2}
\end{theorem}
...
\begin{problem}[Final problem]
    ...
    \begin{enumerate}
        ...
        \item Show that $\operatorname{diam}(M) = \frac{\pi}{\sqrt{k}}$
    \end{enumerate}
    \label{prob:20}
\end{problem}
...
$\operatorname{diam}(M) = \frac{\pi}{\sqrt{k}}$, then $M$ is isometric to $\mathbb{S}^n (\frac{1}{\sqrt{k}})$.

\begin{proof}[of \autoref{thm:13.2}]
    Showing $d(p,q) \leq \frac{\pi}{\sqrt{k}}$ $\forall p,q \in M$ implies $\operatorname{diam}(M)\leq \frac{\pi}{\sqrt{k}}$.
    ...
    $\{E_1(t), \dots, E_{n-1}(t), \gamma'(t)\}$ is an orthonormal basis of $T_{\gamma(t)}M$.
    Set: $X_i(t):=\sin\left(\frac{\pi}{\gamma}t\right)E_i\implies (\nabla_{\gamma}X_i)(t) = \frac{\pi}{\ell} \cos\left(\frac{\pi}{\ell}t\right)E_i(t)$.
    Let $\sigma_i(t):= \operatorname{span}_{\mathbb{R}}\{\gamma'(t), E_i(t)\}$. Then,
    \[
    \begin{aligned}
        \langle R(X_i(t)), \gamma'(t) \gamma'(t), X_i(t) \rangle = \sin^2\left(\frac{\pi}{\ell}t\right)K_{\sigma_i(t)},
    \end{aligned}
    \]
    yielding,
    \[
    \begin{aligned}
        I_{\gamma}(X_i, X_i) = \int_0^{\ell} \{(\frac{\pi}{\ell})^2 \cos^2(\frac{\pi}{\ell}t)-\sin^2\left(\frac{\pi}{\ell} t\right)K_{\sigma_i(t)} \}dt.
    \end{aligned}
    \]
    Summing over $i=1, \dots, n-1$ and using \autoref{prop:9.7}, $Ric \geq (n-1)k,$ and $\ell > \frac{\pi}{\sqrt{k}}$
    \[
    \begin{aligned}
        \sum_{i=1}^{n-1} I_{\gamma} (X_i, X_i) =& \int_0^{\ell} \{ (n-1)(\frac{\pi}{\ell})^2 \cos^2\left(\frac{\pi}{\ell}t\right)\\
        &-\sin^2\left(\frac{\pi}{\ell}t\right) Ric(\gamma'(t), \gamma'(t))\}dt \\
        \leq& \int_0^{\ell} \{(n-1)\left(\frac{\pi}{\ell}\right)^2 \cos^2\left(\frac{\pi}{\ell} t\right) - (n-1)k \sin^2\left(\frac{\pi}{\ell}t\right)\} dt \\
