...
\begin{definition}[Geodesically completeness]
    $M$ is geodesically complete if  the domain of every geodesic can be extended to $\mathbb{R}$ (i.e. $J_v = \mathbb{R}$).
    Alternativly, $M$ is a geodesically complete if $\exp_p$ is defines on $T_pM$ (i.e. $D_p = T_pM$) $\forall p \in M$.
...
\begin{lemma}\label{lemma:11.2}
    Let $M$ be geodesically complete.
    For $p \in M$, by \autoref{theorem:10.10}, $\exists r >0$ s.t.
    $\exp_p : B(0_p, r) \to B(p,r)$ is a diffeomorphism and there exists a unique minimizing geodesic from $p$ to any point in $B(p,r)$.
    For $\rho \in \mathbb{R}$ with $0<\rho<r$, any $q \in M \backslash  B(p,\rho)$
    admits $\hat{p} \in \partial B(p,\rho)$ s.t. $\rho + d(\hat{p}, q) = d(p,q)$
\end{lemma}

\begin{proof}
    $\partial B(p,\rho)= \exp_p (\partial B(0_p,\rho))$ is cpt.
    Since $f(x) := d(q,x)$ is continous (cf. \autoref{prop:open_in_manifold}),
    $f$ attains a minimum on $\partial B(p,\rho)$. Let $\hat{p} \in \partial B(p,\rho)$
    ...
    $h(t) := d(p, c(t))$ is continous with $h(0) =0$ and $h(l) = d(p,q) > \rho$.
...
In particular, if $M$ is geodesically complete, any two points in $M$ can be connected by a minimizing geodesic.
\end{theorem}

\begin{proof}
fix $r>0$ and $0 < \rho <r$ for $p \in M$ as in \autoref{lemma:11.2}.
Assume $q \notin B(p,\rho)$ by taking a smaller $\rho$ if necessary.
By \autoref{lemma:11.2}, $\exists \hat{p} \in \partial B(p,\rho)$ such that $\rho + d(\hat{p},q) = d(p,q)$.
...
We show $\gamma_v |_{[0,d(p,q)]}$ is the minimzing geodesic from $p$ to $q$.
...
\end{equation}
By the triangle inequality $d(p,q) \leq d(p, \hat{p}_1) + d(\hat{p}_1,q)$ and \autoref{eq:11.1} $d(p, \hat{p}_1) \geq d(p, p_1) + \rho_1$
...
\begin{problem}
    ...
    In a metric space $(X,d)$, $(X,d)$ is said to be complete if for all Cauchy sequences is a convergent sequence.
    A subset $A \subset X$ is bounded if $\sup_{p,q \in A} d(p,q) < + \infty$
\end{problem}

\begin{theorem}[Hopf-Rinow Theorem]\label{theorem:11.4}
    The following are equivalent:
    \begin{enumerate}[label=(\roman*), ref=Theorem \thetheorem~(\roman*)]
        \item \label{thm:11.4.1} $M$ is a geodesically complete
        \item \label{thm:11.4.2} $\forall$ bounded closed subset of $M$ is cpt
        \item \label{thm:11.4.3} $(M,d)$ is a complete metric space with respect to Riemannian distance $d$
    \end{enumerate}
\end{theorem}

\begin{proof}
    \autoref{thm:11.4.1} $\implies$ \autoref{thm:11.4.2}: $\forall$ bounded closed set $C \subset M$.
    $\exists r>0$ such that $C \subset B(p,r)$. Since $\bar{B}(0_p,r)\subset T_pM$ is cpt and $\exp_p: T_p M \to M$ is continous, the image
    \begin{equation}\label{eq:11.2}
    \bar{B}(p,r) = \exp_p (\bar{B}(0_p,r))
    \end{equation}
    is cpt.
    Since $C$ is a closed subset of a cpt set, $C$ is cpt.
    \autoref{thm:11.4.2} $\implies$ \autoref{thm:11.4.3}: Let $\{p_i\}^{\infty}_{i=1}$ be a cauchy sequence $\exists i_1 \in \mathbb{N}$ such that $d(p_i, p_j) < 1$ $\forall i,j \geq i_1$ Setting
    \[
    \begin{aligned}
    ...
    Hence $\{p_i\} \subset \bar{B}(p_1,R)$ which is cpt by \autoref{thm:11.4.2}.
...
In the proof \autoref{thm:11.4.3} $\implies$ \autoref{thm:11.4.1} of \autoref{theorem:11.4} the choice of $r>0$ is determined by the bounded set $C$, and no specific
properties of the map $\exp_p : B(0_p,r) \to \exp_p (B(0_p,r))$ are assumed. Consequently, \eqref{eq:11.2} is non-trivial.

\begin{problem}
    Prove \eqref{eq:11.2}. Furthermore, provide an example where \autoref{lemma:11.2} fails when $M$ is not geodesically complete.
    If $M$ is geodesically copmlete (i.e. if the Riemannian distance is complete),
...
\begin{remark}[Remember]
    ...
    In fact, recall: $\forall c^{\infty}$ v.f. on $M$ is complete if $M$ is cpt.
\end{remark}