...
\begin{definition}[Levi-Civita connection]\label{def:levi-civita_connection}
\[
...
\begin{enumerate}[label=(\arabic*)]
    \item \label{item:levi-civita-1} $\nabla_{X+Y}Z = \nabla_X Z + \nabla_Y Z$
    \item \label{item:levi-civita-2} $\nabla_{fX}Y = f\nabla_X Y$
    \item \label{item:levi-civita-3} $\nabla_X(Y+Z) = \nabla_X Y + \nabla_X Z$
    \item \label{item:levi-civita-4} $\nabla_X(fY) = X(f)Y + f\nabla_X Y$
    \item \label{item:levi-civita-5} $\nabla_X Y - \nabla_Y X = [X,Y]$ (torsion-free)
    \item \label{item:levi-civita-6} $X\langle Y, Z \rangle = \langle \nabla_X Y, Z \rangle + \langle Y, \nabla_X Z \rangle$ (metric compatibility)
\end{enumerate}
\end{definition}
...
We start by showing uniqueness. By \autoref{item:levi-civita-6}:
\[
\begin{aligned}
...
\end{aligned}
\]
Summing over these and applying \autoref{item:levi-civita-5}:
...
Thus $\nabla_X Y := \nabla_X^{(U, \phi)}Y$ yields a globally defined vector field. Direct calculation verifies all axioms \autoref{item:levi-civita-1}--\autoref{item:levi-civita-6} of \autoref{def:levi-civita_connection}.
\end{proof}

\begin{problem}
Verify axioms \autoref{item:levi-civita-1}--\autoref{item:levi-civita-6} of \autoref{def:levi-civita_connection} for the Levi-Civita connection $\nabla$ constructed in \autoref{proof:fundamental_theorem_of_riemannian_geometry}.
\end{problem}
...
The second equality follows from \autoref{def:levi-civita_connection} with
...
\autoref{item:christoffel_3} follows from \autoref{def:levi-civita_connection} with $X = \partial_k$, $Y = \partial_i$, $Z = \partial_j$ and \autoref{item:christoffel_2}.
\end{proof}

For local expressions $X = \sum_{i=1}^nX^i\partial_i$ and $Y = \sum_{j=1}Y^j\partial_j$, calculation using \autoref{def:levi-civita_connection} \autoref{item:levi-civita-1}--\autoref{item:levi-civita-4} yields
\[
\begin{aligned}
