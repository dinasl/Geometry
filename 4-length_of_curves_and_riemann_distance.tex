...
\begin{enumerate}[label=(\arabic*), ref=Theorem \thetheorem~(\arabic*)]
...
\begin{lemma}\label{lemma:4.11}
    ...
    Let $C>0$ be the constant from \autoref{lemma:4.10}. Then:
    \begin{enumerate}[label=(\roman*), ref=Lemma \thelemma~(\roman*)]
...
\begin{proof}
    ...
    If $c([0,l]) \subset \phi^{-1}(D)$, let $\bar{c} := \phi \circ c$. By \autoref{lemma:4.10},
    \[
    \begin{aligned}
    ...
    Let $\bar{c}_1 := \phi \circ c_1$. By \autoref{lemma:4.10},
    \[
    \begin{aligned}
    ...
    \end{aligned}\]
    Taking inf over c (using the fact that C in \autoref{lemma:4.10} is independent of c)
    \[
    \begin{aligned}
...
\begin{definition}[Geodesic ball]
    ...
    \end{aligned}
    \]
    is called a \emph{geodesic ball} of radius $r$ centered at $p$
\end{definition}

\begin{proposition}\label{prop:open_in_manifold}
...
\begin{proof}
    ...
    $\exists \gamma > 0$ s.t. $D_i = \bar{B}(x_0, 3r) \subset \phi(O \cap U)$. For the constant $C>0$ from \autoref{lemma:4.11}, choose $\delta >0$ s.t.
    ...
    For $(U, \phi)$ and $D := \bar{B}(x, 3 r_2)$ let $C>0$ be the constant from \autoref{lemma:4.10}. Set $r:= min{C^{-1}r_1, r_2}$.
    Then $B(p, C r) \subset B(p, r_1) \subset O$. For any $y \in B(x, r)$ set $q := \phi^{-1}(y) \in \phi^{-1}(B(x. r_2)) \subset U$. By \autoref{lemma:4.10}:
    \[
    \begin{aligned}
