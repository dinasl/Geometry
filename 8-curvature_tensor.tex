...
\item For $\xi, X, Y, Z$ as in \autoref{prob:13},
\[
\nabla_X^{\xi}\nabla_Y^{\xi}Z - \nabla_Y^{\xi}\nabla_X^{\xi}Z - \nabla_{[X,Y]}^{\xi}Z = R(d\xi(X),d\xi(Y))Z.
\]
\item For vector fields $X,Y,Z$ on $M$
\[
R(X,Y)Z = \nabla_X\nabla_YZ - \nabla_Y\nabla_XZ - \nabla_{[X,Y]}Z.
\]
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
\item $R(u,v)w = -R(v,u)w$, $P_{jik}^l = -R_{ijk}^l$.
\item $R(u,v) + R(v,w) + R(w,u) = 0$.
\[
R_{ijk}^l + R_{jki}^l + R_{kij}^l = 0 \hfill \text{(first Bianchi identity)}.
\]
\item $\langle R(u,v)x, w \rangle = -\langle R(v,u)w, x \rangle$, $R_{ijkl} = -R_{jikl}$.
\item $\langle R(u,v)w, x \rangle = \langle R(x,x)u, v \rangle$, $R_{ijkl} = R_{klij}$.
\end{enumerate}
\end{problem}