...
For each $p \in M$, let $(U_p, \phi_p)$ be a chart around $p$ as in \autoref{def:properly_disc_transf_group}.
...
\label{lemma:disjoint_or_equal}
\end{lemma}

\begin{proof}
    $\gamma_i \neq \gamma_j \implies \gamma_i^{-1}\circ\gamma_j\neq\operatorname{Id}_M$. Then, by \autoref{def:properly_disc_transf_group},
...
For sufficiently large $i$, we have $t_i \in (t_0, t_0 + \delta_0) \implies c(t_i) \in U$, which contradicts \ref{lemma:1.16:2}.
...
Since $M$ is Haussdorff, the limit is unique, so $\phi^{-1}(x_0) = c(t_0)$. Thus, $c(t_0) = \phi^{-1}(x_0) \in U$, which contradicts \ref{lemma:1.16:1}.
\end{proof}
